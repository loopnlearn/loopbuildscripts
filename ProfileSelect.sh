...
section_separator
echo -e "${INFO_FONT}Add the Profile Feature to Existing Download${NC}"
...
if [ $(basename $PWD) = "LoopWorkspace" ]; then
    ...
    echo -e "${INFO_FONT}Downloading profile customizations, please wait...${NC}"
    cd $mytmpdir
    for i in ${!name[@]}
    do
        if [ -z "${url[$i]}" ]; then
...
# *** End of inlined file: src/ProfileSelect.sh ***
